...
class DataSplit:
    def __init__(self,df, dataset_split_path, split_type, label='label'):
        self.df = df
        self.dataset_split_path = dataset_split_path
        self.split_type = split_type
        self.label = label

    def split(self, id, train_ids, val_ids, test_ids, train_size=0.7):
        split_path = os.path.join(self.dataset_split_path, self.split_type)

        if os.path.exists(split_path):
            print(f"Loading {self.split_type} splits from {split_path}")
        ...
        print(f"Splits saved at {split_path}")

        return df_train, df_val, df_test

    def _random_split(self, train_size):
        random_state = 436346
        # separate training data from the rest
        df_train, temp_data = train_test_split(self.df, train_size=train_size,
                                               stratify=self.df[self.label], random_state=random_state)

        df_val, df_test = train_test_split(
            temp_data, test_size=0.5,  # this splits temp_data equally into val and test
            stratify=temp_data[self.label], random_state=random_state
        )
    ...
    def _split_dataset_by_id(self, id, train_ids, val_ids, test_ids):
        print(f"Cross chromosome split train_ids={train_ids}")
        ...
        if missing_train:
            print(f"Warning: The following {id}s for training do not exist in the dataset:", missing_train)
        if missing_val:
            print(f"Warning: The following {id}s for validation do not exist in the dataset:", missing_val)
        if missing_test:
            print(f"Warning: The following {id}s for testing do not exist in the dataset:", missing_test)
        ...
        if not df_unassigned.empty:
            print(f"Warning: Some rows were not assigned to any split. These rows have {id}s:", df_unassigned[id].unique())
