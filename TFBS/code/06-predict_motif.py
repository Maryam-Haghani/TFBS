...
def run_meme(pos_input_fasta, neg_input_fasta, out_dir, nmotifs=5, seed=0, evt= 0.05, meme_path="meme"):
    ...
    return pwm_file  # Output PWM file
...
def evaluate_fimo(df_ground, fimo_output):
    ...
    try:
        df_predicted = pd.read_csv(fimo_output, sep='\t')
        df_predicted = df_predicted.drop_duplicates(subset="sequence_name")
    except Exception as e:
        logger.log_message(f"Error reading predicted file: {e}. Returning empty DataFrame.")
        df_predicted = pd.DataFrame(columns=["sequence_name"])
...
if __name__ == "__main__":
    ...
    for i in range(1, len(dfs_train)+1):
        ...
        pwm_file = run_meme(train_pos_fasta, train_neg_fasta, current_out_dir, nmotifs=config.nmotifs)
        ...
        for qval in config.qval_thresholds:
            ...
            fimo_output = run_fimo(pwm_file, val_fasta, fimo_out, qval)
            # evaluate resulting motifs based on true motifs
            score = evaluate_fimo(df_val, fimo_output)
            fold_scores[qval].append(score)

    logger.log_message("{fold_scores}\nGetting best threshold across folds...")
    best_qval = max(fold_scores, key=lambda k: np.nanmean([d['F1 Score'] for d in fold_scores[k]]))
    ...
    write_fasta(df_train_val[df_train_val['label'] == 1], whole_train_fasta_pos)
    write_fasta(df_train_val[df_train_val['label'] == 0], whole_train_fasta_neg)

    results = []
    for seed in config.seeds:
        logger.log_message(f"*************   SEED-{seed}   ********************")
        # run meme on whole train data with current seed to get final pwm
        final_pwm_file = run_meme(whole_train_fasta_pos, whole_train_fasta_neg, output_dir, nmotifs=config.nmotifs, seed=seed)
        ...
        parent_fimo_out = os.path.join(output_dir, "final_fimo")
        fimo_out = os.path.join(parent_fimo_out, f"fimo_seed-{seed}")
        fimo_output = run_fimo(final_pwm_file, test_fasta, fimo_out, best_qval)
    ...
    logger.log_message(f"Results on test set based on different seeds has been saved at: {results_csv_file}", use_time=True)
    logger.log_message(f"Results based on FIMO Q-val {best_qval} (having highest mean F1 Score across folds)")